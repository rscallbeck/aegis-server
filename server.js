...
// Serve the static Next.js export when bundled together
...
async function handleCashOut(socket, { token }) {
  if (gameState.status !== 'in-progress') {
    return socket.emit('bet-error', 'No active flight to cash out from!');
  }
  ...
  try {
    ...
    // Capture multiplier synchronously before any await to prevent double-payout race
    const cashOutMultiplier = gameState.multiplier;
    bet.cashedOut = true;
    bet.winnings = bet.betAmount * cashOutMultiplier;

    const { data: profile } = await supabase.from('profiles').select('balance_usd').eq('id', user.id).single();
    if (profile) {
      await supabase.from('profiles').update({ balance_usd: profile.balance_usd + bet.winnings }).eq('id', user.id);
    }
